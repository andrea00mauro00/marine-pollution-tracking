...
def show_reports_page(clients):
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Regional Analysis", "Pollutant Trends", "Sensor Analytics", "System Health"])
    
    # Tab 1: Regional Analysis
    with tab1:
        st.markdown("<h2 class='sub-header'>Regional Pollution Analysis</h2>", unsafe_allow_html=True)
        ...
        metrics = timescale_client.get_pollution_metrics(days=days, as_dataframe=False)
        ...
        if metrics:
            ...
            for metric in metrics:
                ...
                regions[region]['avg_risk_scores'].append(metric.get('avg_risk_score', 0))
                regions[region]['max_risk_scores'].append(metric.get('max_risk_score', 0))
                regions[region]['sensor_counts'].append(metric.get('sensor_count', 0))
                regions[region]['times'].append(metric.get('time', ''))
            
            # Calculate regional averages
            region_summaries = []
            for region, data in regions.items():
                avg_risk = sum(data['avg_risk_scores']) / len(data['avg_risk_scores']) if data['avg_risk_scores'] else 0
                max_risk = max(data['max_risk_scores']) if data['max_risk_scores'] else 0
                avg_sensors = sum(data['sensor_counts']) / len(data['sensor_counts']) if data['sensor_counts'] else 0
            ...
            # Create bar chart
            st.markdown("<h3>Average Risk Score by Region</h3>", unsafe_allow_html=True)
            ...
            if hotspots:
                st.markdown("<h3>Current Hotspots by Region</h3>", unsafe_allow_html=True)
                ...
                hotspots_df = pd.DataFrame([
                    {
                        ...
                        'radius_km': float(h.get('radius_km', 1)),
                ...
                fig = px.scatter_mapbox(
                    ...
                    color="severity",
                    size="radius_km",
                    hover_data=["pollutant_type", "status"],
                    color_discrete_map={
                        "high": "#F44336",
                        "medium": "#FF9800",
                        "low": "#4CAF50"
            ...
            st.markdown("<h3>Regional Metrics Summary</h3>", unsafe_allow_html=True)
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "avg_sensors": st.column_config.NumberColumn(
            ...
            st.info("No regional pollution metrics available")
